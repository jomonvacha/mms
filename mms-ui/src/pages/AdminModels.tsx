    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[rgb(var(--text-muted))] inline-flex items-center gap-2">
            <Sparkles size={12} /> {t('adminModels.eyebrow')}
          </div>
          <h1 className="text-[24px] leading-tight font-semibold text-[rgb(var(--text-primary))] mt-1.5">
            {t('adminModels.title')}
          </h1>
      ...
      {/* Tabs */}
      <div className="flex items-center gap-1 border-b border-[rgb(var(--border-subtle))]">
      ...
      {tab === 'registry' ? (
        loading ? (
          ...
          <div className="card p-12 text-center">
            <Cpu size={28} className="mx-auto text-[rgb(var(--text-muted))] mb-3" />
            <p className="text-sm font-medium text-[rgb(var(--text-primary))]">{t('adminModels.empty.noModels')}</p>
      ...
      <div className="card overflow-hidden hidden md:block">
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-[rgb(var(--surface-2))] text-[11px] uppercase tracking-wider text-[rgb(var(--text-muted))]">
              <tr>
            ...
            <tbody className="divide-y divide-[rgb(var(--border-subtle))]">
              {models.map((m) => (
                <tr key={m.code} className="hover:bg-[rgb(var(--surface-2))] transition-colors">
                  <td className="px-4 py-3 align-top min-w-0">
                    <div className="font-medium text-[rgb(var(--text-primary))]">{m.displayName}</div>
                    <div className="text-[11px] text-[rgb(var(--text-muted))] font-mono">{m.code}</div>
                    {m.description && <div className="text-xs text-[rgb(var(--text-muted))] mt-1 max-w-md">{m.description}</div>}
                  </td>
                  <td className="px-4 py-3 align-top">
                    <span className="inline-flex items-center gap-1.5 text-xs text-[rgb(var(--text-secondary))]">
                  ...
                  <td className="px-4 py-3 align-top">
                    <span className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--text-muted))]">
                      {m.lockStyle || 'LOCK'}
                    </span>
                  </td>
                  <td className="px-4 py-3 align-top text-[12px] tabular-nums text-[rgb(var(--text-muted))]">
                    {m.sortOrder ?? 100}
                  </td>
        ...
        {models.map((m) => (
          <li key={m.code} className="card p-4">
            <div className="flex items-start justify-between gap-2 flex-wrap">
              <div className="min-w-0">
                <div className="font-medium text-[rgb(var(--text-primary))]">{m.displayName}</div>
                <div className="text-[11px] text-[rgb(var(--text-muted))] font-mono mt-0.5">{m.code}</div>
              </div>
              <StatusChip status={m.status} />
            </div>
            {m.description && (
              <p className="text-xs text-[rgb(var(--text-muted))] mt-2">{m.description}</p>
            )}
            <div className="flex items-center gap-4 mt-3 flex-wrap">
              ...
              <span className="text-[11px] tabular-nums text-[rgb(var(--text-muted))]">
                sort {m.sortOrder ?? 100}
              </span>
            </div>
            <div className="mt-3 pt-3 border-t border-[rgb(var(--border-subtle))] flex items-center gap-1 flex-wrap">
              {rowActions(m)}
            </div>
          </li>
        ))}
...
function LevelDots({
  ...
  return (
    <div
      className="inline-flex items-center gap-1 text-[10.5px] text-[rgb(var(--text-muted))]"
      role="img"
    ...
    >
      ...
      <div className="flex items-center gap-0.5" aria-hidden="true">
        {[1, 2, 3, 4].map((i) => (
          <span
            key={i}
            className={`w-1.5 h-1.5 rounded-full ${i <= n ? 'bg-brand-500' : 'bg-[rgb(var(--border-subtle))]'}`}
          />
        ))}
      </div>
    </div>
              ...
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-1.5 text-xs text-left transition-colors ${
                  s === current
                    ...
                    : 'text-[rgb(var(--text-secondary))] hover:bg-[rgb(var(--surface-3))]'
                }`}
              >
        ...
        {[1, 2, 3, 4].map((i) => (
          <button key={i} type="button" onClick={() => onChange(i)}
            className={`flex-1 h-9 rounded-lg border text-sm font-medium transition-colors ${
              value === i
                ? 'border-brand-500 bg-brand-50 text-brand-700 dark:bg-brand-900/30 dark:text-brand-300'
                : 'border-[rgb(var(--border-strong))] text-[rgb(var(--text-muted))] hover:bg-[rgb(var(--surface-3))]'
            }`}>{i}</button>
        ))}
      </div>
    </div>
  ...
  return (
    <Modal title={t('adminModels.bindings.title', { name: model.displayName })} onClose={onClose} wide>
      ...
      {loading ? (
        ...
        >
          {categories.map((cat) => (
            <div key={cat.code} className="rounded-lg border border-[rgb(var(--border-subtle))] overflow-hidden">
              <div className="px-4 py-2 bg-[rgb(var(--surface-2))] flex items-center justify-between">
                <span className="text-sm font-semibold text-[rgb(var(--text-primary))]">{cat.displayName || cat.code}</span>
                <span className="text-[11px] uppercase tracking-wider text-[rgb(var(--text-muted))]">{cat.code}</span>
              </div>
                    ...
                    <div key={tier.tierCode}
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          <div className="text-[11px] text-[rgb(var(--text-muted))] font-mono">{tier.tierCode}</div>
                        </div>
                        {isDefault && (
      ...
      <div className="flex items-center justify-end gap-2 pt-4 mt-4 border-t border-[rgb(var(--border-subtle))]">
  ...
  return (
    <Modal title={t('adminModels.providers.title')} onClose={onClose}>
      <div className="space-y-3">
        {rows.map((p, idx) => (
          <div key={p.code} className="flex flex-wrap items-center gap-2 p-3 rounded-lg border border-[rgb(var(--border-subtle))]">
            <span className="text-xs font-mono text-[rgb(var(--text-muted))] w-24 truncate">{p.code}</span>
            <input
            ...
            />
            <label className="inline-flex items-center gap-1.5 text-xs text-[rgb(var(--text-secondary))]">
              <input
                type="checkbox"
                checked={p.enabled}
        ...
        <div className="flex flex-wrap items-center gap-2 p-3 rounded-lg border border-dashed border-[rgb(var(--border-strong))]">
  ...
  return (
    <div className="space-y-3">
      ...
      {loading ? (
        ...
        <div className="card p-8 text-center text-sm text-[rgb(var(--text-muted))]">
              ...
              <tbody className="divide-y divide-[rgb(var(--border-subtle))]">
                {rows.map((r) => (
                  <tr key={r.id} className="align-top">
                    <td className="px-4 py-2.5 text-[12px] text-[rgb(var(--text-muted))] tabular-nums whitespace-nowrap">
                    ...
                    <td className="px-4 py-2.5 text-xs font-mono text-[rgb(var(--text-secondary))]">{r.modelCode || '—'}</td>
                    <td className="px-4 py-2.5 text-xs text-[rgb(var(--text-muted))]">
                      {r.binding ? `${r.binding.categoryCode}/${r.binding.tierCode}` : '—'}
                    </td>
                    <td className="px-4 py-2.5 text-xs text-[rgb(var(--text-muted))] font-mono">{r.ip || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <ul className="md:hidden space-y-2" aria-label="Audit events">
            {rows.map((r) => (
              <li key={r.id} className="card p-3">
                ...
                <div className="mt-2 text-xs text-[rgb(var(--text-secondary))] font-mono truncate">
                  {r.modelCode || '—'}
                  {r.binding && <span className="text-[rgb(var(--text-muted))]"> · {r.binding.categoryCode}/{r.binding.tierCode}</span>}
                </div>
                <div className="mt-2 flex items-center gap-2 flex-wrap text-[11px] text-[rgb(var(--text-muted))]">
