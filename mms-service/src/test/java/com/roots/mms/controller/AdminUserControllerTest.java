...
class AdminUserControllerTest extends AbstractIntegrationTest {
  ...
  void setup() throws Exception {
    createUser("admin", "admin@example.com", "adminpass", List.of(ERole.ROLE_ADMIN, ERole.ROLE_MEMBER));
    adminToken = loginAndGetToken("admin", "adminpass");
    targetUserId = createUser("u2", "u2@example.com", "p2", List.of(ERole.ROLE_MEMBER)).getId().toString();
  }
  ...
  void testUpdateRolesAndStatus() throws Exception {
    ...
    rolesReq.setRoles(List.of("ROLE_MEMBER", "ROLE_MODERATOR"));
